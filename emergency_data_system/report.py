"""
增强报表导出模块
基于 openpyxl 生成多 Sheet 格式化报表，包含数据汇总、统计图表、合规格式

功能:
    1. 多 Sheet 导出 —— 清洗数据 / 异常日志 / 统计汇总 一个文件
    2. 自动格式化 —— 表头加粗、列宽自适应、冻结首行
    3. 条件标注 —— "待补充"单元格橙色加粗（继承 data_cleaner 的逻辑）
    4. 合规页码 —— 页眉页脚（可选）、打印区域设置

使用方式:
    from report import export_full_report
    export_full_report(data_df, anomaly_logs, stats, output_path)
"""
